...
final class ExportServiceTests: XCTestCase {
    ...
    func testJSONExportNilMoodOmitsMoodKeys() {
        // Swift Codable encodeIfPresent omits optional keys when nil — mood/moodValue absent in JSON
        let entry = makeEntry(mood: nil)
        guard let url = ExportService.exportAll(entries: [entry], format: .json) else {
            XCTFail("Export returned nil"); return
        }
        ...
        XCTAssertFalse(first.keys.contains("mood"), "mood key should be omitted when entry has no mood")
        XCTAssertFalse(first.keys.contains("moodValue"), "moodValue key should be omitted when entry has no mood")
        // Required non-optional fields should always be present
        XCTAssertNotNil(first["id"], "id should always be present")
        XCTAssertNotNil(first["isPinned"], "isPinned should always be present")
    }

    func testJSONExportWithTagsIncludesTagNames() {
        let entry = makeEntry()
        let tag1 = Tag(name: "work", colorHex: "#0000FF", isAutoGenerated: false)
        let tag2 = Tag(name: "health", colorHex: "#00FF00", isAutoGenerated: false)
        entry.addTag(tag1)
        entry.addTag(tag2)
        guard let url = ExportService.exportAll(entries: [entry], format: .json) else {
            XCTFail("Export returned nil"); return
        ...
        guard let data = try? Data(contentsOf: jsonFile),
              ...
              let tags = first["tags"] as? [String] else {
            XCTFail("Cannot parse JSON tags"); return
        }
        XCTAssertEqual(tags.count, 2, "Should export both tags")
        XCTAssertTrue(tags.contains("work"), "Should include work tag")
        XCTAssertTrue(tags.contains("health"), "Should include health tag")
    }

    func testJSONExportWithInsightsIncludesInsightArray() {
        let entry = makeEntry()
        entry.addInsight(EntryInsight(type: .sentiment, content: "Positive outlook"))
        entry.addInsight(EntryInsight(type: .suggestion, content: "Try journaling more"))
        guard let url = ExportService.exportAll(entries: [entry], format: .json) else {
            XCTFail("Export returned nil"); return
        }
        defer { try? FileManager.default.removeItem(at: url) }
        ...
        guard let data = try? Data(contentsOf: jsonFile),
              let parsed = try? JSONSerialization.jsonObject(with: data) as? [[String: Any]],
              let first = parsed.first,
              let insights = first["insights"] as? [[String: Any]] else {
            XCTFail("Cannot parse JSON insights"); return
        }
        XCTAssertEqual(insights.count, 2, "Should export both insights")
        XCTAssertEqual(insights[0]["type"] as? String, "sentiment")
        XCTAssertEqual(insights[0]["content"] as? String, "Positive outlook")
        XCTAssertEqual(insights[1]["type"] as? String, "suggestion")
    }

    func testJSONExportIsPinnedTrue() {
        let entry = makeEntry()
        entry.isPinned = true
        guard let url = ExportService.exportAll(entries: [entry], format: .json) else {
            XCTFail("Export returned nil"); return
        ...
        XCTAssertEqual(first["isPinned"] as? Bool, true, "isPinned should be true in JSON when entry is pinned")
    }
}
