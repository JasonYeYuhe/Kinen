...
struct HandoffReportPDFView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            if !report.userTopics.isEmpty {
                section(title: String(localized: "handoff.topics.header")) {
            ...
            if !report.topThemes.isEmpty {
                section(title: String(localized: "handoff.toggle.themes")) {
                    VStack(alignment: .leading, spacing: 4) {
                        ForEach(report.topThemes, id: \.name) { theme in
                            HStack {
                                Text(theme.name).font(.system(size: 12, weight: .medium))
                                Spacer()
                                Text(String(format: String(localized: "handoff.pdf.entryCount.%lld"), theme.count))
                                    .font(.system(size: 11))
            ...
            if !report.cognitiveDistortions.isEmpty {
                section(title: String(localized: "handoff.pdf.section.cognitivePatterns"),
                        subtitle: String(localized: "handoff.pdf.section.cognitivePatterns.subtitle")) {
                        ...
                        ForEach(report.cognitiveDistortions, id: \.name) { d in
                            VStack(alignment: .leading, spacing: 2) {
                                HStack {
                                    ...
                                    Spacer()
                                    Text(String(format: String(localized: "handoff.pdf.entryCount.%lld"), d.count))
                                        .font(.system(size: 11))
                                        .foregroundStyle(.secondary)
                                }
            ...
            if !report.crisisFlags.isEmpty {
                section(title: String(localized: "handoff.pdf.section.crisisFlags"),
                        subtitle: String(localized: "handoff.pdf.section.crisisFlags.subtitle")) {
            ...
            if !report.highlightedEntries.isEmpty {
                section(title: String(localized: "handoff.toggle.entries")) {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(String(localized: "handoff.title"))
                .font(.system(size: 24, weight: .bold))
            Text("\(Self.dateFormatter.string(from: report.dateRange.start)) – \(Self.dateFormatter.string(from: report.dateRange.end))")
                .font(.system(size: 14))
                .foregroundStyle(.secondary)
            Text(String(format: String(localized: "handoff.pdf.generated.%@"), Self.datetimeFormatter.string(from: report.generatedAt)))
                .font(.system(size: 10))
    ...
    private var footer: some View {
        VStack(alignment: .leading, spacing: 4) {
            Divider()
            Text(String(localized: "handoff.pdf.disclaimer"))
                .font(.system(size: 9))
                .foregroundStyle(.tertiary)
        }
        .padding(.top, 8)
    ...
    private var overviewSection: some View {
        section(title: String(localized: "handoff.pdf.section.overview")) {
            let o = report.overview
            VStack(alignment: .leading, spacing: 4) {
                statRow(String(localized: "handoff.pdf.stat.entries"),
                        String(format: String(localized: "handoff.pdf.stat.entries.value"),
                               o.totalEntries, report.dateRange.dayCount, o.writingDays))
                statRow(String(localized: "handoff.pdf.stat.totalWords"), "\(o.totalWords)")
                if let avg = o.averageMood {
                    statRow(String(localized: "handoff.pdf.stat.avgMood"), String(format: "%.2f / 5", avg))
                }
                if let s = o.averageSentiment {
                    statRow(String(localized: "handoff.pdf.stat.avgSentiment"), String(format: "%+.2f (range -1.0 to +1.0)", s))
                }
                if let high = o.highestMoodDay {
                    statRow(String(localized: "handoff.pdf.stat.highestMood"), Self.dateFormatter.string(from: high))
                }
                if let low = o.lowestMoodDay {
                    statRow(String(localized: "handoff.pdf.stat.lowestMood"), Self.dateFormatter.string(from: low))
                }
                if o.longestSilence > 1 {
                    statRow(String(localized: "handoff.pdf.stat.longestSilence"),
                            String(format: String(localized: "handoff.pdf.stat.longestSilence.value.%lld"), o.longestSilence))
                }
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 6) {
                ...
                if let mood = entry.mood {
                    Text(String(format: String(localized: "handoff.pdf.moodOf5.%lld"), mood))
                        .font(.system(size: 10))
                        .foregroundStyle(.secondary)
                }
                if let s = entry.sentiment {
                    Text(String(format: String(localized: "handoff.pdf.sentiment.%@"),
                                String(format: "%+.2f", s)))
                        .font(.system(size: 10))
                        .foregroundStyle(.secondary)
                }
    ...
    private func reasonLabel(_ reason: HandoffReport.HighlightedEntry.HighlightReason) -> String {
        switch reason {
        case .lowestMood: String(localized: "handoff.pdf.reason.lowestMood")
        case .highestMood: String(localized: "handoff.pdf.reason.highestMood")
        case .largestDeviation: String(localized: "handoff.pdf.reason.notable")
        case .crisis: String(localized: "handoff.pdf.reason.crisis")
        case .userPinned: String(localized: "handoff.pdf.reason.pinned")
        }
    }
