    ...
    static func restoreBackup(data: Data, password: String, context: ModelContext) throws -> Int {
        ...
        for backupTag in backup.tags {
            ...
            do {
                ...
                logger.error("Failed to fetch tag '\(backupTag.name)': \(error)")
                existing = []
            }
            if existing.isEmpty {
        ...
        for backupEntry in backup.entries {
            ...
            do {
                ...
                logger.error("Failed to check duplicate for entry \(uuid): \(error)")
                existing = []
            }
            ...
            if existing.isEmpty {
                ...
                entry.isPinned = backupEntry.isPinned ?? false
                entry.previousContent = backupEntry.previousContent
                ...
                for tagName in backupEntry.tagNames {
                    ...
                    do {
                        ...
                        logger.error("Failed to fetch tag '\(tagName)' for entry: \(error)")
                    }
                }
